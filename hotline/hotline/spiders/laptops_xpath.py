...
class LaptopsXpathSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for item in items:
            # Знаходимо назву
            name = item.xpath('.//a[contains(@class,"item-title")]/text()').get()
            # url
            url = item.xpath('.//a[contains(@class,"item-title")]/@href').get()
            # та ціну
            price = item.xpath('.//*[@class="list-item__value-price"]/text()').get()
            # url картинки
